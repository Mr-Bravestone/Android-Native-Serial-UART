package mr.bravestone.serialport;
...
import mr.bravestone.serialport.stk.AbsStickPackageHelper;
import mr.bravestone.serialport.stk.BaseStickPackageHelper;
import mr.bravestone.serialport.utility.ByteUtil;
...
public class SerialTool {
    ...
    private String sPort = "/dev/ttyMSM0";
    ...
    private class ReadThread
        ...
        public void run() {
            ...
            while (!isInterrupted()) {
                try {
                    if (SerialTool.this.mInputStream == null) {
                        return;
                    }

                    byte[] buffer = getStickPackageHelper().execute(SerialTool.this.mInputStream);
                    if (buffer != null && buffer.length > 0) {
                        ComBean ComRecData = new ComBean(SerialTool.this.sPort, buffer, buffer.length);
                        SerialTool.this.onDataReceived(ComRecData);
                    }
...
//                    }
    ...
    private class SendThread
        ...
        public void run() {
            ...
            while (!isInterrupted()) {
                ...
                SerialTool.this.send(SerialTool.this.getbLoopData());
                try {
                    Thread.sleep(SerialTool.this.iDelay);
                } catch (InterruptedException e) {
                    e.printStackTrace();
                }
            }
    ...
    public void onDataReceived(ComBean paramComBean) {
